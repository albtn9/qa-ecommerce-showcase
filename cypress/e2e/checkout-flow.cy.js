describe('Checkout flow', () => {
  ...
  it('confirms order and navigates back to products', () => {
    ...
    cy.get('@alert').should('have.been.calledWith', '✅ Pedido realizado com sucesso!')
    cy.url().should('include', '/products')
  })

  it('shows error feedback when API is unavailable during order creation', () => {
    cy.intercept('POST', 'http://localhost:3001/orders', { forceNetworkError: true }).as('createOrder')
    cy.get('[data-testid="input-name"]').type('Usuario Teste')
    cy.get('[data-testid="input-address"]').type('Rua Central, 100')
    cy.get('[data-testid="input-city"]').type('Sao Paulo')
    cy.get('[data-testid="input-zip"]').type('01001000')
    ...
    cy.get('[data-testid="btn-confirm-order"]').click()
    cy.wait('@createOrder')
    cy.get('@alert').should('have.been.calledWith', 'Erro ao confirmar pedido. Verifique se a API está ativa.')
